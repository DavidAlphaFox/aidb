...
fetch(ModelName, ID, State) ->
  try
    ...
    {{ok, transform_wakeup(result_to_model(Result, Fields))}, State}
  catch
...
find_by(ModelName, Conditions, [], Limit, Offset, State) ->
  MatchSpec = build_match_spec(ModelName, Conditions),
  Transaction0 = fun() ->  mnesia:select(ModelName, MatchSpec) end,
  TransactionL = fun() ->
  ...
  case mnesia:transaction(Transaction) of
    ...
    {atomic, Results} ->
      Schema = ai_db_schema:schema(ModelName),
      Fields = schema_field_names(Schema),
      Models = [transform_wakeup(result_to_model(Result, Fields)) || Result <- Results],
...
count_by(ModelName, Conditions, State) ->
  MatchSpec = build_match_spec(ModelName, Conditions),
  Transaction = fun() -> erlang:length(mnesia:select(ModelName, MatchSpec)) end,
  case mnesia:transaction(Transaction) of
    {aborted, Reason} -> {{error, Reason}, State};
...
transform_sleep(Model) ->  ai_db_transform:model(fun sleep/4,Model).
...
transform_wakeup(Model) -> ai_db_transform:model(fun wakeup/4, Model).
...
build_match_spec(ModelName, Conditions) ->
  ...
  Schema = ai_db_schema:schema(ModelName),
  Fields = schema_field_names(Schema),
  FieldsMap = maps:from_list([field_tuple(I, Fields) || I <- lists:seq(1, erlang:length(Fields))]),
  ...
  OrderingFun = fun(A, B) ->
                    ...
                    erlang:list_to_integer(ANumber) =< erlang:list_to_integer(BNumber)
                end,
...
schema_field_names(Schema) ->
  [ai_db_schema:field_name(Field) || Field <- schema_fields(Schema)].
...
place_id_first([Field|Fields], Acc)->
  case lists:member(id, ai_db_schema:field_attrs(Field)) of
...
condition_to_guard({'or', [Expr1]}, FieldsMap) -> condition_to_guard(Expr1, FieldsMap);
condition_to_guard({'or', [Expr1 | Exprs]}, FieldsMap) ->
  { 'orelse',
    condition_to_guard(Expr1, FieldsMap),
    condition_to_guard({'or', Exprs}, FieldsMap)
  };
condition_to_guard({'not', Expr}, FieldsMap) -> {'not', condition_to_guard(Expr, FieldsMap)};
...
maybe_log(Fun, Args, #state{verbose = true}) -> error_logger:info_msg(log_format(Fun), Args);
maybe_log(_, _, _) -> ok.
